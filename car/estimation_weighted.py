...
def convert_input(X, y_weighted):
    X_diff = [[X[i][1][1:] - X[i][0][1:], X[i][2][1:] - X[i][0][1:]] for i in range(len(X))]
...
def loss_function(params, x_all, y_all_weighted):
    ...
    # y_all: (N,)
    # utility_all: (N, M + 1)
    utility_all = torch.sum(params * x_all, 2)
...
def optimize(params, X, y_weighted, display=True, seed=0):
...
def fit(X, y_weighted, seed=0):
    ...
    optimize(params, X, y_weighted, display=False, seed=seed)
